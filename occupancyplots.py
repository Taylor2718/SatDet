...
for i in ourbands:
    ...
    # plt.show()
#%%
occupancy_per_band_antenna = []
for i in range(len(ourbands)):
    widths_list = widths_antenna[i]
    peak_widths_in_band = widths_list[0]
    occupied = sum(peak_widths_in_band)
    occupancy = occupied/len(waterfalls)
    occupancy_per_band_antenna.append(occupancy)
#%%Reading data
...
for i in ourbands:
    ...
    peaks, _ = find_peaks(filtered_mean_waterfalls, height=max_noise+1, width=8, distance = 120)
    widths = scipy.signal.peak_widths(filtered_mean_waterfalls, peaks, rel_height=0.5)
    widths_satillite.append(widths)
    # plt.plot(filtered_dates, filtered_mean_waterfalls)
    # #plt.plot(filtered_dates, filtered_mean_waterfalls[peaks], "x")
    # plt.plot(filtered_dates[peaks], filtered_mean_waterfalls[peaks], "x")
    # #plt.plot(np.zeros_like(filtered_mean_waterfalls), "--", color="gray")
    ...
    # plt.show()
#%%
occupancy_per_band_satillites= []
for i in range(len(ourbands)):
    widths_list = widths_satillite[i]
    peak_widths_in_band = widths_list[0]
    occupied = sum(peak_widths_in_band)
    occupancy = occupied/len(waterfalls)
    occupancy_per_band_satillites.append(occupancy)
...
plt.ylabel("Ratio Of Time Occupied")
plt.title("Occupancy Time Ratio (over 3 Hours")
plt.show()